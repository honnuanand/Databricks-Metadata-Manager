#!/usr/bin/env python3
"""
Execute the Databricks schema setup for Metadata Manager using CLI authentication
This script uses your existing Databricks CLI authentication
"""
...
def get_databricks_client():
    """Initialize Databricks client using CLI authentication."""
    try:
        # Use default authentication (CLI configuration)
        client = WorkspaceClient()
        print(f"✅ Connected to Databricks using CLI authentication")
        return client
        
    except Exception as e:
        print(f"❌ Failed to connect to Databricks: {e}")
        print("Make sure you're logged in with: databricks auth login")
        return None
...
def execute_sql_statement(client, sql, description=""):
    ...
    try:
        ...
        if "already exists" in str(e).lower():
            print(f"✅ {description} - Already exists (OK)")
            return True
        else:
...
def main():
    ...
    sql_statements = [
        # 1. Create schema
        {
            ...
            """,
            "description": "Creating metadata_manager schema in arao catalog"
        },
        
        # 2. Use schema
        {
            "sql": "USE CATALOG arao",
            "description": "Setting active catalog to arao"
        },
        
        {
            "sql": "USE SCHEMA metadata_manager",
            "description": "Setting active schema to metadata_manager"
        },
    ...
    # Insert sample users (handle potential duplicates)
    sample_data_statements = [
        {
            ...
            """,
            "description": "Inserting/updating sample users"
        }
    ]
    ...
    total_statements = len(sql_statements) + len(sample_data_statements) + len(view_statements)
    ...
    print("\n🔗 Next steps:")
    print("   1. Verify in your workspace: https://e2-demo-field-eng.cloud.databricks.com")
    print("   2. Query: SELECT * FROM arao.metadata_manager.users")
    print("   3. Start your backend server with the correct Databricks credentials")
