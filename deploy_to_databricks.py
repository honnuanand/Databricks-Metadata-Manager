#!/usr/bin/env python3
"""
Databricks Deployment Script for Metadata Manager Application
...
class DatabricksDeployer:
    def __init__(self, profile: str = None, app_name: str = None, config_file: str = None):
        ...
        self.frontend_dir = os.path.join(self.project_root, "front-end")  # metadata-manager uses front-end
    ...
    def _find_project_root(self) -> str:
        ...
        if os.path.exists(os.path.join(self.script_dir, "front-end")):
            return self.script_dir
        elif os.path.exists(os.path.join(os.path.dirname(self.script_dir), "front-end")):
    ...
    def _load_config_from_yaml(self):
        ...
        if os.path.exists(config_path):
            try:
                ...
                print(f"Warning: Could not parse {self.config_file}: {e}")
    ...
    def package_backend(self) -> bool:
        ...
        if os.path.exists(app_yaml_src):
            ...
            print(f"Config file not found: {app_yaml_src}")
    ...
    def deploy_app(self, scope_name: str = None) -> bool:
        ...
        if exit_code != 0 and "already exists" not in stderr.lower() and "maximum number of apps" not in stderr.lower():
            print(f"App creation returned: {stderr}")
    ...
    def hard_redeploy(self, skip_secrets: bool = True) -> bool:
        ...
        if app_exists:
            print(f"App '{self.app_name}' exists. Deleting...")
            ...
            print(f"App '{self.app_name}' does not exist. Proceeding with fresh deployment.")
...
def main():
    parser = argparse.ArgumentParser(
        description="Deploy Metadata Manager app to Databricks",
