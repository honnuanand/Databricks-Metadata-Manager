...
Usage:
    python scripts/grant_permissions.py --app-name metadata-manager --profile DEFAULT \
        --catalog arao --schema metadata_manager

    # Dry run (show what would be granted)
    python scripts/grant_permissions.py --app-name metadata-manager --profile DEFAULT \
        --catalog arao --schema metadata_manager --dry-run
"""
...
def run_command(command: list, profile: str = None) -> Tuple[bool, str, str]:
...
def grant_catalog_permissions(
    ...
    for grant in grants:
        ...
        for priv in privileges:
            ...
            if not success:
                # Try alternative method using SDK
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Grant permissions to metadata-manager app
  python scripts/grant_permissions.py --app-name metadata-manager --profile DEFAULT \\
      --catalog arao --schema metadata_manager
  ...
  python scripts/grant_permissions.py --sp-id 12345678-1234-1234-1234-123456789abc \\
      --profile DEFAULT --catalog arao --schema metadata_manager
"""
    )
